...
for ii = 1 : 6
    ...
    mkdir( [ 'DifferenceConditionalForecasts' CountryName ] );
    cd( [ 'DifferenceConditionalForecasts' CountryName ] );
    ...
    savefig( [ 'Difference Conditional Forecasts for ' CountryName ] );    
    saveas( Figure, [ 'Difference Conditional Forecasts for ' CountryName ], 'emf' );
...
function PlotForecast( Title, VariableName, GrowthRate, Scale, t0, T, oo_, cond, Limits )
    ...
    plot( T, Cond( 1 : L ) * Scale - Uncond( 1 : L )  * Scale, '-b' );
    title( Title );
    
    ToSave = [ T, Cond( 1 : L ) - Uncond( 1 : L ) ];
